...
stats AS (
    SELECT
        *,
        ...
        AVG(ctr) OVER (PARTITION BY hotel_id) AS avg_ctr,
        STDDEV_SAMP(ctr) OVER (PARTITION BY hotel_id) AS sd_ctr,

        AVG(cpc) OVER (PARTITION BY hotel_id) AS avg_cpc,
        STDDEV_SAMP(cpc) OVER (PARTITION BY hotel_id) AS sd_cpc,

        AVG(lr) OVER (PARTITION BY hotel_id) AS avg_lr,
        STDDEV_SAMP(lr) OVER (PARTITION BY hotel_id) AS sd_lr,

        AVG(cpl) OVER (PARTITION BY hotel_id) AS avg_cpl,
        STDDEV_SAMP(cpl) OVER (PARTITION BY hotel_id) AS sd_cpl,
        
        AVG(roas) OVER (PARTITION BY hotel_id) AS avg_roas,
        STDDEV_SAMP(roas) OVER (PARTITION BY hotel_id) AS sd_roas,
        
        AVG(aov) OVER (PARTITION BY hotel_id) AS avg_aov,
        STDDEV_SAMP(aov) OVER (PARTITION BY hotel_id) AS sd_aov
...
SELECT
    ROW_NUMBER() OVER (ORDER BY anomaly_score DESC, month) AS anomaly_rank,
    hotel_id,
    country,
    region,
    hotel_category,
...
FROM scored
ORDER BY anomaly_score DESC, month
LIMIT 15;